...
start = time.time()
input_feature = keras.layers.Input(shape=[80, 96])
...
def build_model():
    ...
    # hidden25 = keras.layers.Conv3D(16, (1, 1, 1), activation='relu', padding='valid', data_format='channels_last',name='layer_con3')(hidden22)
    ...
    # #
    # hidden100 = keras.layers.Reshape((80, 96))(hidden26)
    # hidden101 = keras.layers.Permute((2, 1))(hidden100)
    #
    # hidden01 = keras.layers.Flatten()(hidden101)
    ...
    hidden52 = Bidirectional(LSTM(128, name='layer_lstm2', return_sequences=True))(hidden51)
    hidden01 = keras.layers.Flatten()(hidden52)
...
if __name__ == '__main__':
    ...
    path = 'BottleTrain2D.csv'
    inputs_train = pd.read_csv(path, header=None)
    x_train_input = inputs_train.values

    path = 'BottleTest2D.csv'
    inputs_test = pd.read_csv(path, header=None)
    x_test_input = inputs_test.values

    # X_train = x_train_input.reshape(x_train_input.shape[0], 96, 80)
    # X_train = X_train.transpose(0, 2, 1)
    # X_train = X_train.reshape(X_train.shape[0] * 80, 16, 12, 1)
    # X_test = x_test_input.reshape(x_test_input.shape[0], 96, 80)
    # X_test = X_test.transpose(0, 2, 1)
    # X_test = X_test.reshape(X_test.shape[0] * 80, 16, 12, 1)

    X_train = x_train_input.reshape(23130, 80, 96)
    X_test = x_test_input.reshape(5760, 80, 96)

    reduce_lr = LearningRateScheduler(scheduler)
    m.fit([X_train], [y_train], epochs=100, batch_size=256, callbacks=[reduce_lr], verbose=1)
    m.save('Bottle02.h5')
